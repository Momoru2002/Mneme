...
#[tauri::command]
pub async fn wells_list(db: State<'_, Db>) -> Result<WellListResponse, String> {
    let conn = lock_db(&db)?;
    core_wells::wells_list(&conn)
}

#[tauri::command]
pub async fn wells_get(db: State<'_, Db>, id: String) -> Result<Well, String> {
    let conn = lock_db(&db)?;
    core_wells::wells_get(&conn, &id)
}

#[tauri::command]
pub async fn wells_add(db: State<'_, Db>, input: AddWellInput) -> Result<Well, String> {
    let conn = lock_db(&db)?;
...
#[tauri::command]
pub async fn wells_remove(db: State<'_, Db>, id: String) -> Result<(), String> {
    let conn = lock_db(&db)?;
    wells::remove(&conn, &id).map_err(|e| e.client_message())
}

#[tauri::command]
pub async fn wells_activate(db: State<'_, Db>, id: String) -> Result<(), String> {
    let conn = lock_db(&db)?;
    core_wells::wells_activate(&conn, &id)
}

#[tauri::command]
pub async fn wells_validate(path: String) -> Result<WellValidationResult, String> {
    Ok(wells::validate_well_path(&path))
}

#[tauri::command]
pub async fn wells_browse_host(path: String) -> Result<HostBrowseResponse, String> {
    wells::browse_host(&path).map_err(|e| e.client_message())
}

#[tauri::command]
pub async fn wells_host_home() -> Result<HostBrowseResponse, String> {
